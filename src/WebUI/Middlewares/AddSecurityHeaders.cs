namespace CleanArchitecture.WebUI.Middlewares;

public class AddSecurityHeadersMiddleware
{
    private readonly RequestDelegate _next;
    private readonly ILogger<AddSecurityHeadersMiddleware> _logger;

    public AddSecurityHeadersMiddleware(RequestDelegate next, ILogger<AddSecurityHeadersMiddleware> logger)
    {
        _next = next;
        _logger = logger;
    }
...
public static class AddSecurityHeadersMiddlewareExtension
{
    public static IApplicationBuilder UseAddSecurityHeaders(this IApplicationBuilder app, IConfiguration configuration)
    {
        ...
        if (!isAddSecurityHeaders)
            return app;
        return app.UseMiddleware<AddSecurityHeadersMiddleware>();
    }
}